...
def buildGUI():
            ...
            if frameDict["contrast"].grid_info(): # If contrast options are active, verify text boxes
                ...
                if not error:
                    ...
                    contrastDict["Calculated contrast step ratio: "]["var"].set\
                        ((contrastDict["Minimum contrast ratio (0-100): "]["var"].get()/ \
                            contrastDict["Maximum contrast ratio (0-100): "]["var"].get()) ** (1/(contrastDict["Number of contrast steps: "]["var"].get()-1)))
            ...
            if not error and statusLabel is not None:
                ...
                if proceed:
                    if uploadButton['text'] == "Upload":
                        ...
                        killFlag.put(1)
                        protocolThread = threading.Thread(target=uploadProtocol, 
                                                          args=(frameDict, 
                                                                entryDict, 
                                                                contrastDict, 
                                                                frequencyDict, 
    ...
    def loadPreset():
        ...
        if True:
            ...
            for dict in [entryDict, contrastDict, frequencyDict]:
...
######################################## DEFAULT PROTOCOLS ########################################
            ...
            # On day 1 and 2, reward never times out
            if presetID <= 2:
        ...
        while True:
            ...
            if not error and len(post_mount_locations) - len(pre_mount_locations) == 1: # If new partition is found, save file to new partition
                ...
                if re.match(rf"^CAGE [1-{len(cageList)}][AB]$", driveName): # Check that USB has valid name
                    ...
                    error = True
            elif (not error and len(post_mount_locations) - len(pre_mount_locations) == -1) or \
                    (error and  len(post_mount_locations) - len(pre_mount_locations) == 0): 
                    # If partion was removed, thumb drive was removed so reset partition list
